...
[ -d /sys/firmware/efi ] || (printf 'Non-UEFI boot modes aren'\''t supported! Aborting' && \
    for i in {0..2}; do sleep 1 && printf .; done && exit)
...
basestrap /mnt base base-devel linux-zen linux-firmware man-db man-pages texinfo
...
[[ -z $ans || $ans = [yY] ]] && nano /etc/pacman.conf
